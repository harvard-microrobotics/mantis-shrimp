function [M_q, C_q, G_q, B_q] = get_Lagrangian(z,params)
...
% Added momentum
added_mass_flag = params.added_mass_flag;


if added_mass_flag==1
    added_mass_geometry = params.added_mass_geometry;
    scale_added_mass_length=params.scale_added_mass_length;
    added_mass = params.added_mass*scale_added_mass_length;
    if added_mass_geometry ==1 % Eliptial shape
       added_mass_minor = params.added_mass_minor*scale_added_mass_length;
    else
        added_mass_minor = 0;
    end
    added_mass_b = params.added_mass_b*scale_added_mass_length;
    I_1 = params.I_1+params.added_mass_moment_b*scale_added_mass_length^3;
    I_2 = params.I_2+params.added_mass_moment*scale_added_mass_length^3;
...
m_2 = params.m_2;
% I_2 = params.I_2+params.added_mass_moment;
% I_2 = params.I_2;
x_2 = params.x_2;
y_2 = params.y_2;
...
k_damp = params.k_damp; % air =1 and water =1000
...
kh_2 = params.kh_2;
alpha= params.alpha; %deg2rad(27.9317);% params.alpha;
% gravity
g = params.gravity;
...
G_q = [m_1*g*e_2'*R_th1_JJ*com1 + m_2*g*e_2'*R_th1_JJ*(e_2*L_1+R_th3*com2) + k_damp_linear*dth1 + k_s*(th1-(pi+alpha)); %+k_s*(1-l_s0/l_s)*L_4*d_s*sin(th1-alpha); ...
       -m_2*g*e_2'*R_th1_p_th3*JJ*com2 + k_arm_damp*dth2 + kh_2*(th2-th2h_i)];
...
if added_mass_flag==1
    ...
    Ma = Ja'*Ja*added_mass;
    Ma_b = Ja_b'*Ja_b*added_mass_b;
    M_q = M_q +Ma + Ma_b;

    dM11_dth2 = 2*Ja1*(-L_1*e_1'*R_th3*e_2)*(-1);
    dM12_dth2 = Ja2*(-L_1*e_1'*R_th3*e_2)*(-1);
    % dM21_dth2 = dM12_dth2;

    C_qa = 1/2*[dM11_dth2*dth2   , dM11_dth2*dth1+2*dM12_dth2*dth2;...
                -dM11_dth2*dth1  , 0];
    C_q = C_q+C_qa*added_mass;
    
    %if eliptical
    if added_mass_geometry==1
        Ja1_minor = -L_1*e_2'*R_th3'*e_1+x_2;
        Ja2_minor = -x_2;
        Ja_minor = [Ja1_minor, Ja2_minor];

        Ma_minor = Ja_minor'*Ja_minor*added_mass_minor;

        dM11_dth2_minor = 2*Ja1_minor*(L_1*e_1'*R_th3*e_1)*(-1);
        dM12_dth2_minor = Ja2_minor*(L_1*e_1'*R_th3*e_1)*(-1);
        
        C_qa_minor = 1/2*[dM11_dth2_minor*dth2   , dM11_dth2_minor*dth1+2*dM12_dth2_minor*dth2;...
                         -dM11_dth2_minor*dth1  , 0];
        M_q = M_q + Ma_minor*added_mass_minor;
        C_q = C_q+C_qa_minor*added_mass_minor;
    end
